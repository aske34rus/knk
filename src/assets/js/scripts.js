//- @prepros-prepend ../lib/bootstrap/js/*.js
...
// Кнопка "на верх"
$('body').append('<p id="back-top" class="to-link"><a href="#top"><img src="../img/top-bottom.png"></a></p>');
$ ('#back-top').fadeOut(0);
$(document).ready(function(){
  $ (window).scroll (function () {
...
// Код, получение координат относительно всей страницы
...
$(document).ready(function(){
  if ($(window).width() >= screenSM){
    $('.paralax').mousemove(function(e){
      ...
      var paralax = (10/$(this).width()); // вывод результата в консоль

  // Паралакс эфекты контейнеров на основе коортидан курсора
      $("body").css({
        'background-position': (55-(paralax)*(Xinner))+'% '+-(paralax*(Yinner))*2+'px'
      });
      $(".container-fluid .container").css({
        'top' : (paralax*(Yinner))*2,
        'left': (paralax*(Xinner))*2,
        // 'position': 'relative',
      });
    });
  }
...
$(document).ready(function(){
  ...
  // Паралакс эфекты контейнеров на основе коортидан курсора
      $(".cube .container").css({
        // 'background':'#000'
        'transform': 'rotatex(0deg) rotateY('+Xinner+'deg) rotateZ(-'+Yinner+'deg)'
      });
